...
One logical manager exists per active account/candidate, while operator chat is separate. Fresh research and critic contexts reduce contamination. Material sensing continues every three minutes in Python; if a manager is busy, deltas coalesce for the next turn rather than overlapping. The supervisor backs off 1/5/30 seconds and opens a circuit after three crashes in ten minutes.
...
- The 1/5/30-second crash and three-in-ten-minute circuit behavior exists as a tested policy only. Process ownership, real restart/orphan cleanup, and duplicate-artifact prevention across restart remain WI-001 work.
- The authenticated-smoke runtime is exclusively leased, but an ungraceful `SIGKILL` leaves a fail-closed stale lease that currently requires explicit operator cleanup.
