...
5. Deliver the WI-004 L1 goal as four checkpointed slices in one implementation batch: WI-005 establishes canonical contracts and the deterministic Node-to-Python authority seam; WI-006 connects the real manager and independent critic; WI-007 proves the failure matrix and durable restart idempotency; WI-008 proves material scheduling and the two-hour soak. The implementation agent advances through validated dependencies without intermediate human review, pushing each work-item checkpoint; close WI-004 only after the combined independent review and manual acceptance gate pass.
