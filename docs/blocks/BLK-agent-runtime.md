...
- The only schema source is `contracts/paper-intent/fixture-l1/v1/`, using JSON Schema Draft 2020-12. Every registered schema has a stable `urn:marketpilot:paper-intent-fixture:v1:<name>` `$id`; every top-level artifact or envelope schema also requires `schemaVersion: { "const": 1 }` and `profile: { "const": "marketpilot.paper-intent-fixture.v1" }`. Reusable nested component schemas do not add envelope fields to their declared shapes.
- Every object is closed with `additionalProperties: false`; conditional branches also use `unevaluatedProperties: false`. Validators enable format assertion, resolve only the committed local registry, and reject unknown references. This profile is still a rejected pre-acceptance candidate, so the corrections planned here do not widen an accepted contract. Its freeze begins with the first accepted WI-005 checkpoint; after that, adding a field or enum, relaxing a constraint, or changing semantics requires a new schema/profile rather than widening V1.
- `fixture-meta.schema.json` has `$id = urn:marketpilot:paper-intent-fixture:v1:meta`, requires Draft 2020-12 core/applicator/validation/format-assertion plus `urn:marketpilot:paper-intent-fixture:v1:vocabulary`, and is the `$schema` of every profile schema. The local vocabulary has exactly three assertion keywords: `mpNfc: boolean`; `mpScalarLength`, a closed object requiring nonnegative integer `min` and `max` with `min <= max`, measured in Unicode scalar values; and `mpSortedUniqueBy: string`, whose value is `$value` for scalar arrays or the required string field of object items. Node and Python register the same keywords, fail startup if the vocabulary is unavailable, and pass every positive/negative case in `custom-vocabulary-vectors.json`. No inline artifact validator may independently restate or widen them.
- Before WI-005 can be accepted, `registry.json` contains exactly these schema keys: `Primitives`, `Producer`, `Fact`, `Provenance`, `Policy`, `ResearchEvent`, `CandidateManifest`, `ManagerSemanticDraft`, `TradeIntent`, `CriticSemanticDraft`, `CriticVerdict`, `GateDecision`, `OrderPlan`, `ExecutionEvent`, `AuditEvent`, `AppIncidentEvent`, `FixtureAuthorityRequest`, `FixtureAuthorityResponse`, and `ProtocolError`. The registry, `fixture-meta.schema.json`, every listed schema, and `custom-vocabulary-vectors.json` are committed and hash-inventoried together; none may be deferred past the V1 freeze.
- Strings are valid Unicode 17.0 NFC and are rejected rather than normalized. Both validators fail startup unless their normalization data is exactly Unicode 17.0; runtime-native tables from another Unicode release are not accepted. Opaque IDs, enums, hashes, keys, and reason codes are ASCII. Text is 1–1,024 characters without control characters. Input, authority-audit, and per-scope app-incident collections contain at most 16 entries; a rejection contains 1–26 unique `reasonCodes` drawn from the frozen 26-code set, containing exactly the safely evaluable failures in precedence order. Identifier-keyed collections are unique and already sorted lexicographically. Validators reject unsorted input rather than silently rewriting it.
- The opaque fields `requestId`, `operationId`, `jobKey`, every artifact/fact/provenance/revision/audit/incident ID, and `producer.runId` match `^[a-z][a-z0-9_]{2,63}$` and use the prefixes `req_`, `op_`, `job_`, `re_`, `rev_`, `fact_`, `prov_`, `cand_`, `ti_`, `cv_`, `gd_`, `plan_`, `exec_`, `audit_`, `ae_`, `inc_`, or `run_` according to type. Symbolic fields are separate: `instrumentId` matches `^[A-Z][A-Z0-9.]{0,9}$`, currency matches `^[A-Z]{3}$`, `sourceId` matches `^[a-z][a-z0-9.-]{2,63}$`, and profile/policy/artifact/enum values use their exact declared constants. Hashes are 64 lowercase hexadecimal characters.
...
| `TradeIntent` | committed stand-in for `MANAGER` in WI-005; in WI-006 an app serializer losslessly binds a real manager's semantic draft to immutable IDs, linkage, time, producer identity, and hash | Untrusted input |
| `CriticVerdict` | committed stand-in for `CRITIC` in WI-005; in WI-006 an app serializer losslessly binds a real critic's semantic draft to immutable IDs, linkage, time, producer identity, and hash | Untrusted input |
| `GateDecision`, `OrderPlan`, `ExecutionEvent` | `PYTHON_AUTHORITY` | Python creates; each artifact carries the required producer; Node may validate but never create, amend, or repair |
| `AuditEvent` | implicit `PYTHON_AUTHORITY` ownership; this profile has no producer field on the audit shape | Python creates; Node verifies the fixed event sequence, subjects, hashes, and chain but never creates, amends, or repairs |
| `AppIncidentEvent` | app runtime or fixture store | Records a typed no-exposure app, store, scheduler, conflict, correction, recovery, or cleanup outcome that is outside the authority result, whether it occurs before or after one; its side ledger never replaces or mutates an authority result/audit |

The manager and critic originate every decision-bearing semantic field: disposition or verdict, proposal, thesis or counterargument, reason, and evidence selection. Closed `ManagerSemanticDraft` and `CriticSemanticDraft` schemas live in the same committed local registry. The manager draft contains only `disposition`, nullable `proposal` using the exact `TradeIntent` proposal shape, nullable `abstainReasonCode`, `thesis`, and sorted unique `evidenceFactIds`. `PROPOSE` requires a proposal, null abstain reason, and 1–16 IDs that all exist in the canonical event; `ABSTAIN` requires null proposal, an allowed abstain reason, and 0–16 existing IDs. The critic draft contains only `verdict`, `reasonCode`, `counterargument`, and 1–16 sorted unique existing `evidenceFactIds`; its verdict/reason branches are exactly those of `CriticVerdict`. The app validates each draft, wraps selected facts in the canonical event linkage, and may attach only already-owned identity, linkage, timestamp, producer, and hash fields. It must never provide a completed artifact, preselect the conclusion, ask either role to echo prose or hashes, or alter a semantic field. Accepted/rejected scenario mandates may state the objective and constraints but not the expected disposition, verdict, reason, prose, or serialized answer. The manager receives the canonical `ResearchEvent` through its one paper-fixture MCP read. The critic receives that rights-filtered event, candidate, and bound intent directly in a fresh process with no MCP, discovery, collaboration, transcript, or other model-visible capability.

### Lexical And Hash Rules

Only `schemaVersion`, authority-audit `sequence`, and app-incident `sequence` are JSON numbers. Decision-affecting financial values are strings parsed with Python `Decimal` in a local precision-38 context; no binary float, coercion, global decimal-context change, whitespace, sign where forbidden, exponent, omitted part, leading zero, `NaN`, or infinity is accepted.
...
| `UsdCollarPrice` | `^(0|[1-9][0-9]*)\.[0-9]{4}$` | `0.0001` through `1004999.9998`; output-only for `OrderPlan.priceCollar.maximumLimitPrice` |
...
| --- | --- |
| `ResearchEvent` | `schemaVersion`, `profile`, `artifactType = ResearchEvent`, `eventId`, `eventHash`, `producer`, `instrumentId`, `assetClass`, `currency`, `eventKind`, `revisionId`, nullable `supersedesEventId`, `rightsClass`, `publishedAt`, `observedAt`, `facts`, `provenance`. `eventKind = FIXTURE_ISSUER_NOTICE`; event/fact `rightsClass` values are `PUBLIC_OFFICIAL`, `LICENSED_MODEL_OK`, or `LOCAL_RESTRICTED`, ordered exactly as `PUBLIC_OFFICIAL < LICENSED_MODEL_OK < LOCAL_RESTRICTED`. Provenance `sourceClass` is separately frozen as `PUBLIC_OFFICIAL`, `LICENSED_VENDOR`, or `LOCAL`, mapping respectively to those three rights classes. `facts` contains one each of `NOTICE_TEXT`, `REFERENCE_PRICE_USD`, and `ASK_PRICE_USD`, with `factId`, `kind`, typed `value`, `rightsClass`, and `provenanceId`. Each provenance entry has `provenanceId`, `sourceId`, `sourceClass`, `sourceRef`, `sourceRevision`, `publishedAt`, `retrievedAt`, and `contentHash`; every fact reference resolves, every provenance entry is referenced, and event rights equal the strictest mapped right across the event, every fact, and every provenance entry. |
...
| `AppIncidentEvent` | `schemaVersion`, `profile`, `artifactType = AppIncidentEvent`, `incidentId`, `incidentHash`, `scopeType`, `scopeId`, `sequence`, `occurredAt`, `boundary`, `code`, `exposureEffect = NONE`, nullable `subjectHash`, and nullable `previousIncidentHash`. `scopeType = OPERATION` requires an `op_` scope ID and uses the canonical request `decisionAt` (the claimed stored request when present); `scopeType = JOB` requires a `job_` scope ID and uses its `scheduledAt`. Sequence starts at 1 per scope and is contiguous; the first previous hash is null and later incidents chain to the prior incident hash. `incidentId = inc_` plus the first 32 hex characters of `SHA-256(ASCII(profile) || 0x00 || ASCII(scopeId) || 0x00 || ASCII(base-10 sequence) || 0x00 || ASCII(boundary) || 0x00 || ASCII(code))`. The fixture-only app/store path creates and append-only persists it; it contains no prompt, response prose, raw exception, path, credential, account data, or input excerpt. |

The closed L1 incident-code set is `AUTH_REQUIRED`, `RATE_LIMITED`, `SOL_ULTRA_UNAVAILABLE`, `MODEL_REROUTED`, `THREAD_POLICY_INVALID`, `MCP_CONTRACT_INVALID`, `MCP_TOOL_FORBIDDEN`, `MANAGER_OUTPUT_INVALID`, `CRITIC_OUTPUT_INVALID`, `MANAGER_ABSTAINED`, `CRITIC_NOT_DISTINCT`, `AUTHORITY_INPUT_ERROR`, `AUTHORITY_TIMEOUT`, `AUTHORITY_PROCESS_FAILED`, `AUTHORITY_OUTPUT_INVALID`, `AUTHORITY_RESPONSE_MISMATCH`, `TURN_TIMEOUT`, `TURN_PROTOCOL_FAILED`, `TURN_SCHEMA_INVALID`, `CLIENT_CLEANUP_FAILED`, `RUNTIME_NOT_QUALIFIED`, `NON_PUBLIC_DATA_REJECTED`, `PARTIAL_FILL_UNSUPPORTED`, `CORRECTION_REQUIRES_NEW_OPERATION`, `OPERATION_IN_PROGRESS`, `IDEMPOTENCY_CONFLICT`, `RECOVERY_INCOMPLETE`, `SCHEDULER_CIRCUIT_OPEN`, `SCHEDULER_RESULT_INVALID`, `SCHEDULER_RECOVERY_ABSTAINED`, `SCHEDULER_LEASE_UNCERTAIN`, and `SCHEDULER_STATE_INVALID`. Unexpected internal faults are not coerced into this set.

| Incident boundary | Allowed codes |
| --- | --- |
...
A proposed intent's single canonical event reference must contain exactly the three required `NOTICE_TEXT`, `REFERENCE_PRICE_USD`, and `ASK_PRICE_USD` fact IDs. A nonempty proper subset is structurally valid but yields the domain rejection `INTENT_EVIDENCE_MISMATCH`; an unknown fact ID or wrong event/hash yields the same code after schema/hash validation. This domain rule is Python-only and gives the `partial_evidence` scenario an exact oracle.
...
Python computes the BUY collar as `referencePrice * (1 + buyCollarRatio)`, quantized down to four places with `ROUND_DOWN`. `OrderPlan.priceCollar.maximumLimitPrice` uses the output-only `UsdCollarPrice` primitive so the maximum valid input price and frozen ratio deterministically produce the representable upper bound `1004999.9998`; all input prices and the actual plan limit remain `UsdPrice`. The plan limit is the lesser of that collar and the intent's maximum entry price. Risk uses the exact unrounded `quantity * collar`, not reported notional, and rejects values above USD 100.00. The gate also requires `askPrice <= plan limit`; an accepted request then creates one plan and fills the full quantity once at `askPrice`. `fillNotionalUsd` is reported to two places with `ROUND_HALF_EVEN`; risk never uses the rounded report. An impossible post-accept simulator condition is an internal invariant failure, never a fabricated fill or second decision.
...
The Node launcher enforces transport/shallow-envelope safety, executes the committed Draft 2020-12 schemas for non-authoritative deep structural/lexical validation, and passes every bounded syntactically valid request to Python as the exact original bytes without repairing artifacts. Python independently executes the same committed local schema registry before its semantic gate; handwritten validators may enforce semantic invariants but may not duplicate or widen structural schema rules. Node does not evaluate input linkage, time, rights, critic, or policy rules. It independently validates response schema, producer ownership, hashes, conditional shape, and references only to decide whether Python's entire response can be accepted as an authority result. The launcher accepts one request per process, enforces a two-second absolute deadline and the same output bound, and uses exactly `AUTHORITY_INPUT_ERROR`, `AUTHORITY_TIMEOUT`, `AUTHORITY_PROCESS_FAILED`, `AUTHORITY_OUTPUT_INVALID`, or `AUTHORITY_RESPONSE_MISMATCH` as redacted no-exposure adapter codes. Timeout, signal, nonzero exit, extra/malformed output, request-ID/reference mismatch, or response/hash/schema failure discards every returned domain artifact. The Python process performs no network access and reads or writes no product state.
...
5. Deliver the WI-004 L1 goal as four checkpointed slices in one implementation batch: WI-005 establishes canonical executable contracts and the deterministic Node-to-Python authority seam; WI-006 connects model-originated manager and critic drafts through role-specific zero-excess-capability runtimes; WI-007 proves real failure paths plus atomic, non-replaying durable recovery at materially distinct artifact/audit commits; WI-008 makes the scheduler consume that durable result boundary and proves the two-hour local-authority soak plus a separate hosted walkthrough. The implementation agent advances through validated dependencies without intermediate human review, pushing each work-item checkpoint; close WI-004 only after the combined independent review and manual acceptance gate pass.
...
- A fixture-only simulation implementation exists, but the first WI-004 independent review rejected its L1 claim because schemas were not executable authority, hosted roles echoed prebuilt conclusions, durable recovery replayed acceptance and was unsafe under concurrent claims, and scheduler/cleanup paths could report false success. WI-005 through WI-008 are reopened as one corrective batch; none of this fixture code is production trading capability.
