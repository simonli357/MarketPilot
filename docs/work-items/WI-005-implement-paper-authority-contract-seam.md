...
Accepted corrective evidence, 2026-08-07:

- The raw-byte-inventoried local registry executes all 19 schemas and 22 Unicode-17 custom-vocabulary vectors in Ajv and Python jsonschema with asserted UTC format, exact local references, closed branches, frozen producer ownership, and no retrieval fallback. Startup-tamper tests passed 10/10, including unknown/external references, vocabulary/format changes, duplicate IDs, path substitution, and uninventoried bytes.
- `npm run test:paper-core` passed 78/78. Its shared differential corpus passed 1,477 structural and lexical vectors in both runtimes, covers all registered schemas and 26 ordered domain codes, and includes coherent response forgeries, decimal/rounding boundaries, provenance rights, Unicode-version regressions, registry tampering, canonical stdio, and hostile process outcomes. The focused Python authority and transport suites passed 37/37.
- The two-second launcher ceiling is enforced at both call sites. After reusing one pre-crawled packaged meta-validator, 10 accepted cold processes measured 0.708 s median/0.957 s maximum and 10 rejected cold processes measured 0.876 s median/1.168 s maximum under host load, with canonical output, empty stderr, and no failed launch.
- `npm run check:paper-core`, `npm run audit:paper-core`, `npm run test:codex`, and `npm audit` passed; the Codex regression suite passed 315/315 and both dependency audits reported zero known vulnerabilities. Ruff check/format, Python byte-compilation, JavaScript syntax checks, and diff/whitespace checks passed on the corrective scope.
- Sequential manual `paper:fixture` runs produced the accepted seven-event gate/plan/fill report and the rejected five-event `QUANTITY_LIMIT_EXCEEDED`, `NOTIONAL_LIMIT_EXCEEDED` report with null plan/execution. Only the committed synthetic `PUBLIC_OFFICIAL` MPTEST fixture was used; no secrets, account data, broker, licensed data, product state, persistence, or runtime network access entered the authority path.
