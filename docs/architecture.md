...
| Fixture contract validation | Ajv 8.20.0 in Node; jsonschema 4.26.0, referencing 0.37.0, and unicodedata2 17.0.1 in Python | Execute the same committed Draft 2020-12 registry independently on both sides of the L1 authority boundary; pin Python NFC data to Unicode 17.0 to match the qualified Node 22 runtime. | Exact npm lock plus hash-locked Python 3.12 graph under `requirements/`. | Reviewed 2026-08-07: all four upstream projects are active, unarchived, and had maintained releases in the preceding year; Node 22/Python 3.12 checks and both vulnerability audits pass. Ajv/jsonschema/referencing and their transitive graph are MIT except fast-uri (BSD-3-Clause), typing-extensions (PSF-2.0), and unicodedata2 (Apache-2.0). Integration is limited to the two local schema adapters, one custom vocabulary, one normalization-data provider, and isolated setup/audit environments; there is no runtime retrieval. | `registry.mjs` and `paper_contract_schema.py` are the single replacement boundary around the frozen schemas and Unicode data; any upgrade reruns parity, normalization, lock, license, and vulnerability gates. |
...
- Recovery: a process-owning supervisor permits one- and five-second automatic app-server retries, opens a latched circuit on the third crash in ten minutes with a 30-second minimum reset cooldown, and starts a fresh client only after the prior controlled POSIX process group is proven absent. Accepted and uncertain artifact keys are fenced for that supervisor process; Python remains responsible for durable cross-application idempotency. Deliberate `setsid()` escapes and a supervisor killed with `SIGKILL` require a future service/cgroup or surviving-launcher containment boundary. A recovered app-server receives a fresh physical thread rehydrated from encrypted logical memory rather than resuming Codex-owned sensitive persistence. Python or broker failure suspends exposure and reconciles; unknown fills/orders/positions and invariant breaches enter operational kill.
