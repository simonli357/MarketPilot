...
const environment = path.join(root, ".venv-paper-audit");
const python = path.join(environment, "bin", "python");
const lock = path.join(root, "requirements", "paper-audit.lock");
...
process.stdout.write(`paper audit environment ready: ${path.relative(root, python)}\n`);
